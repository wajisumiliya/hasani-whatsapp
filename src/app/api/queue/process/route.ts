...
export async function POST(req:Request){
  ...
  const {data:rows,error}=await db.rpc('wa_claim_queue_batch',{p_limit:limit});
  if(error)return NextResponse.json({error:error.message},{status:500});
  const provider=getWhatsAppProvider();let sent=0,failed=0;
  for(const r of rows||[]){
    try{
      const {data:campaign,error:campaignError}=await db.from('wa_campaigns').select('status,wa_templates!inner(template_name,language_code)').eq('id',r.campaign_id).single();
      if(campaignError||!campaign||campaign.status!=='processing')throw new Error('Campaign is not processing');
      const rawTemplate:any=campaign.wa_templates;const template=Array.isArray(rawTemplate)?rawTemplate[0]:rawTemplate;
